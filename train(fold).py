# -*- coding: utf-8 -*-
"""3D_Train(Fold).ipynb의 사본

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1rElYQDFA71dJS0qW3unuJNMpIDxzReqq
...
training_transform = tio.Compose([
    ...
    tio.RandomAffine(p=0.5,degrees=(60,)),
    ZNormalization(),
...
path='/content/drive/MyDrive/data/3d data/test models/'
use_amp = True
save_path='/content/drive/MyDrive/data/3d data/test models/'
scaler = torch.cuda.amp.GradScaler(enabled=use_amp)
early_stopping = EarlyStopping(patience = CFG['PATIENCE'], verbose = True, path =save_path )

def train(model, optimizer, train_loader, val_loader, scheduler, device, fold_index):
    model.to(device)
    ...
    for epoch in range(1, CFG['EPOCHS']+1):
        ...
        early_stopping(-val_acc, model, fold_index=fold_index)
...
for fold_index, (trn_idx, val_idx) in enumerate(kfold.split(all_df),1):
    print("fold_index",fold_index)
    train_df = all_df.iloc[trn_idx]
    valid_df  = all_df.iloc[val_idx]
    ...
    train(model, optimizer, train_loader, val_loader, scheduler, device, fold_index)